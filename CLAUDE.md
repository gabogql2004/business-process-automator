...
- **Registro cerrado en producción con `REGISTRATION_CODE`** (opcional, vacío por defecto): como el registro es público y cada ejecución de un nodo de IA gasta la `ANTHROPIC_API_KEY` del dueño del proyecto, dejar el registro completamente abierto en un deploy público significaría que cualquiera puede crear una cuenta y generar cargos en la cuenta de Anthropic del desarrollador. Con esta variable seteada, `POST /api/auth/register` exige que el body incluya `codigoInvitacion` igual a ese valor (403 si no matchea). En desarrollo local se deja sin setear para no interponer fricción. Complementario, no sustituto, de poner un límite de gasto en la consola de Anthropic — la variable evita el abuso más obvio pero no protege contra un token filtrado ni contra el propio dueño de la cuenta pasándose de la raya.
